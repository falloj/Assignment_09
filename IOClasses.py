...
# JFallon, 2022-Mar-27, added code to save track data to file, load inventory
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        #fileCD = file_name[0])
        #fileTrack = file_name[1]
        try:
            with open(file_name[0], 'w') as fileCD:
                for disc in lst_Inventory:
                    fileCD.write(disc.get_record())
            fileCD.close()
            # TODone add code to save track data to file
            with open(file_name[1], 'w') as fileTrack:
                for disc in lst_Inventory:
                    for track in disc.cd_tracks:
                        eachTrack = str(disc.get_record().strip()+','+track.get_record().strip()+'\n')
                        fileTrack.write(eachTrack)
            fileTrack.close()
